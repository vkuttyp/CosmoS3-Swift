...
func loadSettings() -> AppSettings {
    ...
    return AppSettings(port: 18003, storageBaseDir: "./disk",
                       connectionString: "host=localhost port=3306 database=cosmos3 username=root password=root",
                       requireAuth: false, logging: true, cors: true)
}

let settings = loadSettings()
print("[CosmoS3Host.MySQL] Starting on port \(settings.port)...")

let rawDb = try await DatabaseFactory.create(type: .mysql, connectionString: settings.connectionString)
try await DatabaseFactory.ensureSchema(db: rawDb, type: .mysql)
...
app.get("/_cosmo/health") { ctx in
    try ctx.response.writeJson(["status": "ok", "db": "mysql", "version": "1.0.0"])
}

print("[CosmoS3Host.MySQL] Listening on http://0.0.0.0:\(settings.port)")
try await app.run()
